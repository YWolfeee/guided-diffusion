...
import torch
import argparse
from guided_diffusion.celebA import faceid
...
@torch.no_grad()
def compute_faceid(target: str, sample_batch: str, sample_num: int):
    model = faceid.load_arcface('/home/linhw/code/guided-diffusion/ckpts/celebA/model_ir_se50.pth', 'cuda')

    ref_data = Image.open(target).convert('RGB')
    ref_data = torch.tensor(np.array(ref_data).transpose(2, 0, 1)).unsqueeze(0) / 127.5 - 1
    ref_data = ref_data.cuda().float()
    
    sample_data = np.load(sample_batch)['arr_0']
    sample_data = sample_data.transpose(0, 3, 1, 2) / 127.5 - 1
    sample_data = torch.tensor(sample_data).cuda().float()
    bs = 64

    ref_feature = faceid.arcface_forward(model, ref_data)

    sample_feature = []
    for idx in range(0, len(sample_data), bs):
        # (bs, 3, 256, 256)
        feature = faceid.arcface_forward(model, (sample_data[idx:idx+bs]))
        sample_feature.append(feature)
    sample_feature = torch.cat(sample_feature, dim=0)
    
    return torch.linalg.norm(sample_feature - ref_feature, dim=1).mean().item()

@torch.no_grad()
def compute_kid(ref_batch: str, sample_batch: str, sample_num: int):
    bs = 64

    ref_batch = np.load(ref_batch)['arr_0'][:sample_num]
...
def main(args: argparse.Namespace):
    ...
    print("FID", FID, "sFID", sFID)
    
    print("compute kid..")
    kid, _ = compute_kid(args.ref_batch, args.sample_batch, sample_num=1000)
    print("kid:", kid)

    print("compute faceid..")
    faceid = compute_faceid(args.target, args.sample_batch, sample_num=1000)
    print("faceid", faceid)
